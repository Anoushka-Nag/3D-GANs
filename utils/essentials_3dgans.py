...
def train_discriminator(
    ...
    for batch_idx, real_x in enumerate(data_loader):
        ...
        fake_x = noise_generator(batch_size).to(device)
        fake_x = generator(fake_x)
        ...
        real_loss = criterion_discriminator(real_out, real_y)
        fake_loss = criterion_discriminator(fake_out, fake_y)
...
def train_generator(
    ...
    running_loss = 0.0
    for batch_idx, fake_x in enumerate(data_loader):
        batch_idx = batch_idx + 1
        ...
        fake_out = generator(fake_x)
        fake_out = discriminator(fake_out)
